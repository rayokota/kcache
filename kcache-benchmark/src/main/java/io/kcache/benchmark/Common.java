 ...
 *
 * <p>
 * Members do not reflect the typical code standards of the LmdbJava project due to compliance
 * requirements with JMH {@link Param} and {@link State}.
 */
@State(Benchmark)
...
public class Common {
    ...
    /**
     * Keys are always an integer, however they are actually stored as integers (taking 4 bytes) or
     * as zero-padded 16 byte strings. Storing keys as integers offers a major performance gain.
     */
    @Param("true")
    boolean intKey;

    /**
     * Determined during {@link #setup(BenchmarkParams)} based on {@link #intKey} value.
     */
    int keySize;
    /**
    ...
    /**
     * Whether the values contain random bytes or are simply the same as the key. If true, the
     * random bytes are obtained sequentially from a 1 MB random byte buffer.
     */
    @Param("false")
    boolean valRandom;
